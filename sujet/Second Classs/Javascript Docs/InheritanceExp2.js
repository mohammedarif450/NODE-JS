...
//private - functions are declared inline inside the object's constructor 
...
//privileged - methods are declared with this.methodName=function(){...}
//				and may invoked by code external to the object.

//public - properties are declared with this.variableName and may be read/written 
//			from outside the object.

//public - methods are defined by Classname.prototype.methodName = function(){...} 
...
//>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>EXAMPLE<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<
//Example
//In this example, a person's name and race are set at birth and may never be changed.

//When created, a person starts out at year 1 and a hidden maximum age is determined for that person. 

//The person has a weight which is modified by eating (tripling their weight) or exercising (halfing it). 

//Every time the person eats or exercises, they grow a year older. 

//The person object has a publicly accessible 'clothing' property which anyone can modify, as well as a dirtFactor 
//which can be modified manually (throwing dirt on or scrubbing it off), but which increases every time the person eats
// or exercises, and is reduced by the use of the shower() method.
...
function RunGavinsLife(){ 
	var gk=new Person("Gavin","Hindu");       //New instance of the Person object created. 
	var lk=new Person("Lisa","Muslim");        //New instance of the Person object created. 
